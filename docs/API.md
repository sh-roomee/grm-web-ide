  ...
  "comments": [
    {
      ...
      "status": { "state": "open", "fileMtime": 1785028901234 }
    }
  ],
...
- `status`는 **넘긴 코멘트가 반영됐는지**를 서버가 판정한 결과다

| state | 뜻 | 근거 |
| --- | --- | --- |
| `open` | 아직 그대로다 | `code` 조각이 지금 파일에 있다 |
| `applied` | 반영된 것으로 보인다 | 그 조각이 사라졌다 (`reason: "changed"`) |
| `frozen` | 판정하지 않는다 | 커밋에 단 코멘트라 대상이 굳어 있다 |
| `unknown` | 판정할 수 없다 | `code`가 비어 있다 |

  줄 번호가 아니라 **코드 조각**으로 찾는다 — 위쪽이 바뀌어 줄이 밀려도 따라간다.
  `side: "left"`(삭제된 쪽)는 방향이 뒤집혀서, 조각이 **되살아나면** `applied`다.
  `fileMtime`은 그 파일이 마지막으로 바뀐 시각(ms)이며 줄 단위가 아니다.

- `?ids=a,b,c`를 주면 **그 코멘트들만으로** `prompt`를 만든다. `comments`는 항상
  전부 돌려준다 — 화면은 목록 전체를 보여주면서 보낼 것만 고르기 때문이다
...
| --- | --- |
| `id` | 하나 지우기 |
| `ids` | 쉼표로 여러 개 (`반영된 N개 정리`) |
| `all=1` | 전부 지우기 |

## GET /api/summary

사이클 요약. 다음 지시를 쓸 때 사람이 다시 훑지 않아도 되게 한 장으로 모은다.

```json
{ "summary": "## 기준점 이후 (22분)\n\n파일 4개 · +18 −10 · 확인 1/4\n…" }
```

**요약을 지어내지 않는다.** 코드가 무엇을 하는지 말하려면 모델이 필요하고 그건
터미널의 AI가 이미 한다. 여기서 모으는 것은 **AI가 알 수 없는 것들**이다.

| 담기는 것 | 어디서 오나 |
| --- | --- |
| 경과 시간 | 기준점을 잡은 시각 (`state.baselineAt`) |
| 파일 수 · `+`/`−` · 확인 진행률 | `git status` + 확인 표시 |
| 아직 확인 안 한 파일 / 확인한 파일 | 확인 표시의 지문이 지금 변경과 같은가 |
| ⚠ 위험 신호 | `/api/risks`와 같은 판별 |
| 아직 반영 안 된 코멘트 | 코멘트 + 반영 판정 |

기준점이 없으면 제목이 `## 지금 워킹트리`가 되고 경과 시간은 빠진다.

## 컨텍스트 바구니

"이 파일도 같이 봐"를 모아 두는 곳. 코멘트가 **판단**을 넘기는 길이라면 이쪽은
**읽을 곳**을 넘기는 길이다. 저장 위치는 코멘트와 같은 `<git-dir>/grmide-state.json`
(`context` 배열)이다.
...
| kind | 필드 | 프롬프트에 담기는 것 |
| --- | --- | --- |
| `file` | `path` | 파일 전체 (600줄까지, 넘으면 잘랐다고 적는다) |
| `range` | `path` `line` `endLine` | 그 구간. 줄 번호를 붙여 담는다 |
| `grep` | `query` | **넘길 때 다시 검색한** 결과 (40건까지) |

**내용은 담을 때가 아니라 넘길 때 읽는다.** 담아 둔 내용을 얼려 두면 AI가 고친 뒤에는
프롬프트가 사실과 달라진다. 바구니는 "무엇을 볼지"만 들고 있다.
...
```json
{
  "items": [
    {
      "id": "xms14lxiehrdv",
      "key": "file:src/lib/noiseCancel.js",
      "kind": "file",
      "path": "src/lib/noiseCancel.js",
      "addedAt": "2026-07-26T01:35:23.366Z"
    }
  ],
  "prompt": "아래 파일을 참고해줘. 지금 내가 보고 있는 곳이다.\n\n## src/lib/noiseCancel.js\n…"
}
```

`prompt`는 목록과 함께 내려준다. 클립보드 복사는 클릭 핸들러 안에서 아무것도 기다릴
수 없기 때문이다 (`await` 하나만 지나도 브라우저가 사용자 제스처를 회수한다).

### POST /api/context

본문은 위 표의 모양이다. `key`가 같은 것이 이미 있으면 새로 넣지 않고
`{ item, added: false }`를 돌려준다 — 같은 파일이 프롬프트에 두 번 들어가지 않게.

| 상태 | 뜻 |
| --- | --- |
| 400 | 모양이 어긋났다 (경로가 비었거나 줄 번호가 1보다 작다) |
| 403 | 저장소 밖의 경로 |
...
- 마크다운(`.md` · `.markdown`)이면 `preview.kind`가 `"markdown"`이다.
  `before`/`after`는 항상 `null` — 바이트 수는 문서 판단에 쓰이지 않는다
- 저장소 밖 경로는 403

## GET /api/blob

미리보기 원본 바이트. 이미지를 `<img src>`로 그릴 때, 그리고 마크다운 원문을 글자로
받을 때 쓴다.
...
마크다운은 `text/markdown; charset=utf-8`로 나간다. 브라우저가 실행하지 않는 타입이고
`nosniff`까지 붙으므로, 이 경로가 HTML을 흘리는 구멍이 되지 않는다. 그리기는 클라이언트가
트리로 한다(`web/src/markdown/`).

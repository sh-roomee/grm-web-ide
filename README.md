...
git 뷰어로 시작했지만 지금은 그보다 넓다 — 코드를 읽고(⌘P·⌘⇧F·파일 트리),
히스토리를 보고, AI가 방금 무엇을 바꿨는지 가려내고, 리뷰 코멘트를 AI에게 넘기는
일까지 한다. 그래서 이름이 `gitshow`에서 `grmide`로 바뀌었다.

![grmide 기본 화면 — 파일 트리, 변경 파일 목록, side-by-side diff](docs/images/changes-view.jpg)

**IDE를 켜는 이유를 없애는 것**이 목표다. 그래서 기준선은 "편집이 없는 읽기 전용
IDE"다 — 코드를 읽고 찾는 일은 넓게 허용하고, 쓰기는 stage/commit까지만 한다.
원격은 fetch와 fast-forward pull까지만 — 편집기·터미널 임베드·히스토리를 다시
쓰는 조작(push/rebase/reset)은 넣지 않는다.
...
![통합 검색 — 액션·파일 결과를 한 창에서](docs/images/search-everywhere.jpg)
...
검색창에 `확인`·`요약`·`blame`·`비교`·`fetch` 같은 이름을 치면 **액션**이 결과로
나오고 `Enter`로 실행된다. 기능이 늘 때마다 바에 버튼을 더 박지 않아도 되는
자리이고, 우클릭 `Git ▸` 메뉴도 **같은 목록**에서 만들어진다 — 이름과 동작이 두
곳에 흩어져 어긋나는 일이 없다.

**지금 할 수 있는 것만 나온다.** 담아 둔 것이 없으면 `컨텍스트 바구니`가, 변경이
없으면 `전체 확인`·`요약`이 목록에서 빠진다. 실행해도 아무 일이 없을 것을 보여
주면 검색 결과를 믿을 수 없게 된다.

들어 있는 것: `브랜치 비교` · `Blame 주석`(토글) · `git fetch` · `git pull` ·
`전체 확인` · `기준점 해제` · `사이클 요약` · `리뷰 스레드` · `컨텍스트 바구니` ·
`히스토리/변경사항 전환` · `파일 트리 접기·펼치기` · `코드 글자 크기 기본값`.

### 최근 본 파일 — ⌘E

![최근 본 파일 — 검색어 없이 열면 방금 보던 순서](docs/images/recent-files.jpg)

`⌘E`는 파일 창을 **검색어 없이** 연다. 그러면 목록이 **최근 본 순서**가 된다.
전에는 빈 검색어에 `git ls-files` 순서대로 앞 60개가 나왔는데, 그건 아무 뜻이
없는 목록이라 사실상 검색어를 치기 전까지 빈 화면이었다.

`⌘P`는 이전 검색어를 남긴다(같은 것을 다시 찾는 일이 잦다) — 두 키의 차이가
그것뿐이라, 손이 기억하는 쪽을 누르면 된다.

### 디렉토리 트리

![파일 트리 — 디렉토리를 펼쳐 파일을 읽기 전용으로 연다](docs/images/file-tree.jpg)
...
변경사항과의 경계선을 끌면 트리 높이가 조절되고 기억된다.

### 파일 아이콘

IntelliJ·VSCode의 아이콘 테마가 하던 일이다. 트리·변경 목록·문서 탭·검색 결과
어디서든 파일 이름 앞에 종류별 아이콘이 붙는다 — 사람은 이름을 읽기 전에 색으로
먼저 거르기 때문에, 목록을 훑는 속도가 다르다.

- 그림 파일이 아니라 **약어 + 색 배지**다. `JS` 노랑, `TS` 파랑, `V`(Vue) 초록,
  `MD` 파랑, `{}`(json) 노랑, 디렉토리는 파인더의 파란 폴더.
- **파일명이 확장자보다 먼저다.** `package.json`은 JSON이 아니라 `npm`(빨강)이고,
  `.gitignore`는 `git`(주황)이다.
- **테스트 파일**(`*.test.*` / `*.spec.*`)은 언어 약어에 초록을 입힌다.
- 모르는 확장자도 약어를 만들어 회색으로 붙인다.

종류를 늘리려면 `web/src/icons/index.js` 레지스트리에 한 줄 넣으면 된다 —
문법 강조 플러그인과 같은 구조다.
...
| **↔ 비교** | 이 브랜치가 결국 뭘 바꿨나 (`↔ 비교`로 열었을 때만 나타난다) |

### 문서 탭

![문서 탭 — 워킹트리 diff(M) · 읽기 전용 파일(JS) · 커밋 diff(C)를 동시에](docs/images/tabs-view.jpg)
...
| 좌측 위 | 파일 트리 (경계선을 끌어 높이 조절) |
| 좌측 아래 | 충돌 / Staged / 수정 그룹별 변경 파일 목록 (+추가 −삭제 줄 수) |
| 우측 | diff (나란히 · 한 줄), 문법 강조, 단어 단위 하이라이트, 변경 위치 눈금 |

위 스크린샷에서 좌측 목록을 보면 `ko.json`은 **Staged**, `Display.vue`와
`noiseCancel.js`는 **수정**, `styles.scss`는 `?` 표시(untracked)로 구분되어
있다. 한 파일이 일부만 stage된 경우 양쪽에 모두 나온다.

### 보기 방식 — 나란히 / 한 줄로

![한 줄 보기 — −줄과 +줄이 한 컬럼에 순서대로](docs/images/inline-view.jpg)
...
### 브랜치 비교 — 이 브랜치가 결국 무엇을 바꿨나

![브랜치 비교 — merge-base 기준으로 바뀐 파일과 diff, 코멘트까지](docs/images/compare-view.jpg)

상단 브랜치 이름 옆 **`↔ 비교`** 를 누르면 현재 브랜치를 기본 브랜치와 비교한다.
기준은 자동으로 찾고(원격 기본 브랜치 → main → master), 비교 화면 안에서 브랜치
선택기로 아무 브랜치·태그로나 바꿀 수 있다.

- **merge-base 기준(세 점)이다.** 기준 브랜치가 그동안 전진한 것은 섞이지 않는다 —
  GitHub PR과 같은 의미론이라, AI가 브랜치에 커밋을 쌓은 뒤 **PR 올리기 전 마지막
  확인**이 이 화면이다.
- 좌측에 바뀐 파일 목록, 우측에 diff — 변경사항 화면과 같은 레이아웃이라 문법
  강조·⌘F·한 줄 보기가 그대로 동작한다. 연 파일은 `B` 탭으로 쌓인다.
- **파일이 많아도 찾는 길이 있다.** 목록 위 필터로 이름을 거르고(목록에 포커스가
  있을 때 `⌘F`가 여기로 온다), `⌘P`로 열어도 비교에 포함된 파일이면 **비교
  탭으로** 열린다 — 찾아서 여는 동작이 보던 화면의 상태를 따라간다.
- **커밋된 것만** 본다. 아직 커밋 안 된 수정은 변경사항 화면의 몫이다.
- 코멘트도 그대로 단다 — 브랜치 리뷰에서 발견한 것을 `리뷰 N → 프롬프트 복사`로
  AI에게 넘기는 흐름이 워킹트리 리뷰와 똑같다.
- 상단에 `커밋 N개`와, 기준이 앞서 있으면 그 개수도 알려준다 — pull 할 때가
  됐다는 신호다.
...
위 스크린샷이 그 상태다. 이미 확인한 변경(`isDisabled` 전환 등)은 회색 컨텍스트로
내려가고, 기준점 이후 새로 들어온 `disabled` prop 한 줄만 변경으로 표시된다.
`HEAD 대비`로 바꾸면 둘 다 보인다.
...
상단 `⚠ 2`는 그런 파일이 2개라는 뜻이고, 누르면 **그 파일만** 본다. 목록의 ⚠에 마우스를
...
**Shift 두 번**으로 통합 검색을 열고 `fetch`나 `pull`을 치면 **액션**이 나온다.
Enter로 실행하면 우측 하단에 진행 스피너가 돌고, 끝나면 결과가 잠깐 뜬다 —
"fetch 완료 — 원격에 새 커밋 3개"처럼.

- **fetch**는 `git fetch --prune`. 히스토리 화면의 원격 브랜치가 최신이 된다.
- **pull**은 `git pull --ff-only`. **fast-forward만** 한다 — 로컬과 원격이 갈라져
  있으면 아무것도 바꾸지 않고 실패하고, 머지/리베이스 판단은 터미널에서 하라고
  알린다.

`push` · `rebase` · `reset --hard` 같은 히스토리를 다시 쓰는 조작은 넣지 않았다.
가져오기는 되돌릴 수 있지만 잘못 밀어낸 것은 남의 저장소에 남는다.

### 우클릭 메뉴

![우클릭 메뉴 — 파일 액션과 Git 하위 메뉴](docs/images/context-menu.jpg)

파일 행(트리·변경 목록·비교·커밋 파일)과 코드 영역 어디서든 우클릭하면 컨텍스트
메뉴가 뜬다 — IDE의 편집기 메뉴 자리다.

- 파일 액션: **탭으로 열기 · 컨텍스트에 담기 · stage/unstage(변경 목록에서) · 경로 복사**
- **`Git ▸`** 하위 메뉴: **브랜치 비교 · Blame 주석 · git fetch · git pull**.
  앞으로 파일 히스토리 같은 git 기능이 이 자리에 쌓인다 — 기능이 늘 때마다
  화면에 버튼을 더 박지 않는다.
- 파일에서 `브랜치 비교…`를 고르면 **브랜치 선택 팝업**이 먼저 뜨고, 고르면 비교
  화면으로 가면서 **그 파일**이 열린다. 그 파일에 차이가 없으면 "차이 없음"으로
  알린다 — IntelliJ의 Compare with Branch와 같은 흐름이다.
- **`Blame 주석`**(IntelliJ의 Annotate)을 켜면 줄 왼쪽에 **날짜·작성자**가 붙는다
  (아래 그림). 올리면 커밋 정보(sha·작성자·메시지)가 뜨고, **누르면 히스토리에서
  그 커밋이 열린다.** 아직 커밋 안 된 줄은 `미커밋`으로 표시된다. blame은 지금
  워킹트리 파일 기준이라 파일 뷰어와 워킹트리 diff에서만 켜진다.
- 입력창(찾기·코멘트)에서는 브라우저 기본 메뉴가 그대로 뜬다 — 붙여넣기가 필요하다.

![Blame 주석 — 줄마다 날짜·작성자, 누르면 그 커밋으로](docs/images/blame-view.jpg)

메뉴 항목들은 통합 검색의 **액션**과 같은 목록에서 온다. 손이 어느 쪽에 있든
(마우스든 키보드든) 같은 기능에 닿는다.

### 단축키

| 키 | 동작 |
| --- | --- |
| `Shift` `Shift` | 통합 검색 (파일 · 커밋 · 텍스트 · 액션 한 번에) |
| `⌘P` | 파일 열기 (이전 검색어를 남긴다) |
| `⌘E` | 최근 본 파일 (검색어를 비우고 연다) |
...
| `↑↓` `←→` | 파일 트리에서 행 이동 · 펼치기/접기 (트리에 포커스가 있을 때) |
...
- 파일 하나의 diff가 20,000행을 넘으면 서버가 잘라내고 그 사실을 화면에 알린다.
  그 아래는 **가상 스크롤**로 보이는 만큼만 그린다 — 12,546행 파일에서 열기와
  `⌘F`가 모두 즉시 반응한다. 대신 2,000행을 넘는 파일에서는 화면 밖 줄이 DOM에
  없어서 **그 범위를 넘는 드래그 선택이 끊긴다.** 그 아래 크기는 전부 그리므로
  사람이 읽고 복사하는 파일에서는 이 제약이 없다.

...
![통합 검색 — 파일·텍스트 결과를 한 창에서](docs/images/search-everywhere.jpg)
...
- **`Blame 주석`**(IntelliJ의 Annotate)을 켜면 줄 왼쪽에 **날짜·작성자**가 붙는다.
  올리면 커밋 정보(sha·작성자·메시지)가 뜨고, **누르면 히스토리에서 그 커밋이
  열린다.** 아직 커밋 안 된 줄은 `미커밋`으로 표시된다. blame은 지금 워킹트리
  파일 기준이라 파일 뷰어와 워킹트리 diff에서만 켜진다.
